...
export async function GET(request: NextRequest) {
  ...
  const usosPolo = searchParams.getAll("uso");
  ...
  try {
    ...
    if (searchType === "location") {
      ...
      if (cidades.length > 0) {
        // Removido upper/unaccent para simplicidade com ANY.
        // Se a busca case-insensitive/sem acento for crucial, uma abordagem mais complexa é necessária.
      ...
      // Busca por Polo Agrícola e Uso (single select)
      whereClause += ` AND b.preco IS NOT NULL AND b.area IS NOT NULL AND b.area > 0`;
      if (poloAgricola) {
      ...
      if (usosPolo.length > 0) {
        whereClause += ` AND b.uso = ANY($${paramIndex})`;
        values.push(usosPolo);
        paramIndex++;
      }
    }
    ...
    const query = `
      ...
      LEFT JOIN reserva_legal r
             ON upper(unaccent(r.municipio)) = upper(unaccent(b.municipio))
            AND r.uf = b.uf
